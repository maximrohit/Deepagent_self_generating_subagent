...
3) domain: one of finance | comics | fitness | general (MACRO category)
...
7) matching_agents: EXACT existing agent names from the registry that fit the domain
   (empty if none fit — never force-fit finance agents onto comics)
...
11) proposed_agent_name: MACRO snake_case name if create_needed else ""
    (comics_lore_agent, NOT batman_vs_dhruv_agent)
...
4. Domain match is mandatory — NEVER cross-wire domains:
   - finance → indian_stock_trader_3m / market_analyst / us_stock_trader_3m …
   - comics → comics_lore_agent (CREATE if missing). NEVER finance agents.
   - fitness → home_strength_training_agent (CREATE if missing). NEVER finance.
   - general research → primary_deep_agent
...
9. Agent names must reflect function: finance only for finance; comics only for
   comics; primary only as generic research.
"""
...
class TopicBriefing(BaseModel):
    ...
    domain: Literal["finance", "comics", "fitness", "general"] = Field(
        description="MACRO domain for routing and creation."
    )
...
# Explicit finance asks only — everything else is non-finance (comics, fitness, …).
_FINANCE_TASK_MARKERS = (
    ...
    "cryptocurrency", "bitcoin", "market cap", "earnings", "buy/sell",
    "trade setup", "trading setup", "investment verdict", "bullish", "bearish",
)
_COMICS_TASK_MARKERS = (
    "batman", "superman", "spiderman", "spider-man", "wonder woman", "marvel",
    "dc comic", "dc comics", "superhero", "super hero", "superheroes", "comics",
    "comic book", "comic-book", "commando dhruv", "dhruv", "rajnagar",
    "avenger", "x-men", "manga", "anime", "who would win", "who will win",
)
_FITNESS_TASK_MARKERS = (
    "workout", "exercise", "fitness", "gym", "dumbbell", "resistance band",
    "muscle", "training plan", "yoga", "cardio", "diet", "nutrition",
)
_FINANCE_ONLY_AGENTS = frozenset(
    {
...
_GENERIC_AGENTS = frozenset({"primary_deep_agent", "general_research_agent"})
...
def _classify_task_domain(text: str) -> str:
    """Return finance | comics | fitness | general from the user/TODO text."""
    blob = (text or "").lower()
    if any(m in blob for m in _FINANCE_TASK_MARKERS):
        return "finance"
    if any(m in blob for m in _COMICS_TASK_MARKERS):
        return "comics"
    if any(m in blob for m in _FITNESS_TASK_MARKERS):
        return "fitness"
    return "general"


def _is_finance_task(text: str) -> bool:
    return _classify_task_domain(text) == "finance"
...
def _agent_looks_finance(
    ...
    """True only for finance specialists — not generic/primary research agents."""
    n = (name or "").strip().lower()
    if not n or n in _GENERIC_AGENTS:
        return False
    if n in _FINANCE_ONLY_AGENTS:
        return True
    if any(
        x in n
        for x in (
        ...
        return True
    hay = f"{description} {' '.join(capabilities or [])}".lower()
    if "general-purpose" in hay or "general purpose" in hay or "comics" in hay:
        return False
    # Name-led only for dynamic agents; avoid matching primary via "finance" tool text.
    return False
...
def _agent_domain(
    ...
    if n in _GENERIC_AGENTS or "general-purpose" in hay or "general purpose" in hay:
        return "general"
    if _agent_looks_finance(name, description, capabilities):
        return "finance"
    if any(x in hay for x in ("comic", "superhero", "fiction", "lore_agent")):
        return "comics"
    if any(x in hay for x in ("fitness", "workout", "training", "nutrition", "strength")):
        return "fitness"
    return "general"
...
def _domains_compatible(task_domain: str, agent_domain: str) -> bool:
    if task_domain == agent_domain:
        return True
    # Generic research may backstop non-finance domains when no specialist exists.
    if agent_domain == "general" and task_domain != "finance":
        return True
    # Finance tasks may use primary only as last resort (caller decides).
    if task_domain == "finance" and agent_domain == "general":
        return True
    return False
...
def _proposed_name_for_domain(domain: str, notes: str = "") -> str:
    ...
    defaults = {
        "comics": "comics_lore_agent",
        "fitness": "home_strength_training_agent",
        "finance": "market_research_agent",
        "general": "general_research_agent",
    }
    canonical = defaults.get(domain, "general_research_agent")
    if domain in {"comics", "fitness"}:
        return canonical
    if "CREATE_NEEDED" in (notes or "").upper() and ":" in notes:
        hint = notes.split(":", 1)[-1].strip()
        token = hint.split()[0].strip(",.;").lower() if hint else ""
        # Reject micro / versus / one-off titles.
...
def _find_domain_agent(registry: SubAgentRegistry, domain: str) -> Optional[str]:
    ...
    for meta in registry.all_metadata():
        ...
        elif domain != "finance" and meta.name in _GENERIC_AGENTS:
    ...
    if generics:
        generics.sort()
        # Prefer primary_deep_agent when present.
...
def _resolve_agent_for_todo(
    ...
    *,
    ...
    for meta in registry.all_metadata():
        ...
        if domain in {"comics", "fitness"} and meta.name in _GENERIC_AGENTS:
            continue
        ...
        if domain == "finance":
            ...
            if "intraday" in blob or "trade" in blob:
                if "trader" in meta.name or "trade" in hay:
                    score += 3
        if domain == "comics" and any(x in hay for x in ("comic", "superhero", "lore")):
            score += 8
        if domain == "fitness" and any(
            x in hay for x in ("fitness", "workout", "training")
        ):
            score += 8
        if meta.name in _GENERIC_AGENTS and domain == "general":
            score += 2
        if score > 0:
    ...
    # Non-finance with no specialist → None so execute path CREATE_NEEDED.
    if domain in {"comics", "fitness"}:
        return None
...
def _harden_topic_briefing(
    ...
    llm_domain = (briefing.domain or "general").lower()
    if domain == "general" and llm_domain in {"finance", "comics", "fitness"}:
        domain = llm_domain
    elif domain != "general":
        pass  # keep deterministic
    else:
        domain = llm_domain if llm_domain in {"finance", "comics", "fitness", "general"} else "general"
    ...
    for meta in registry.all_metadata():
        ...
        elif domain == "general" and meta.name in _GENERIC_AGENTS:
            matching_agents.append(meta.name)
    ...
    if flows_manifest is not None:
        for flow in flows_manifest.all_flows():
            ...
            if domain != "finance" and any(
                _agent_looks_finance(a) for a in agents
            ):
                continue
            if domain == "finance" and not any(
                _agent_looks_finance(a) for a in agents
            ):
                # Allow general_web_research for finance as weak match only if no better.
                if flow.flow_id != "general_web_research":
                    continue
            # Comics/fitness: only keep flows whose agents are compatible.
            if domain in {"comics", "fitness"}:
                ok = True
                for a in agents:
        ...
        for fid in briefing.matching_flows:
            if fid not in matching_flows and flows_manifest.get(fid):
                # Only add if still domain-safe.
                flow = flows_manifest.get(fid)
                agents = list(flow.required_agents or []) if flow else []
                if domain != "finance" and any(_agent_looks_finance(a) for a in agents):
                    continue
    ...
    specialist_exists = any(
        _agent_domain(
            m.name, m.description, m.capabilities
        )
        == domain
    ...
    if domain == "comics":
        ...
        if not any("finance" in d.lower() or "stock" in d.lower() for d in dont_list):
            dont_list.append("Do not use stock/finance agents or market framing")
        if not any("batman_only" in d.lower() or "micro" in d.lower() for d in dont_list):
            dont_list.append("Do not create a micro batman_only / dhruv_only agent")
    if domain == "fitness" and not any("stock" in d.lower() for d in dont_list):
        dont_list.append("Do not use finance/stock agents for workout planning")
        ...
        for todo in todos:
            ...
            if meta and _agent_looks_finance(
                ...
                meta_new, status = create_sub_agent_with_critic_loop(
                    f"MACRO {task_domain} domain agent for user task:\n{task}\n"
                    f"Step:\n{todo['description']}",
                    proposed,
                    registry,
                    flows_manifest=flows_manifest,
